module CommentsProcessor
  class NewCommentJob < Struct.new(:comment_item)
    ...
    def perform
      ...
      if comment_item.votes_available? && (comment_item.votes_count != comment.votes.count)
        comment_item.votes.each do |comment_vote|
          ...
          unless Vote.exists?([vote_author.id, comment.id, 'Comment'])
            Delayed::Job.enqueue(VotesProcessor::NewVoteJob.new(vote_author.name, comment_vote.timestamp, comment_vote.weight, comment_vote.rate, comment, "Comment"))
          end
        end
      end
    end
